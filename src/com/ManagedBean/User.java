...
public class User {
	...
	public String getCustomerDetails() {
		 ...
		 try {
			 String sqlQuery = "Select UserId , CustId , Bank from USER_INFO where UserId = '" +this.userId + "'";
		 ResultSet rs = dbController.callDB(sqlQuery);
		 while(rs.next())  {
		 this.setUserId(rs.getString(1));
		 this.setCustomerID(rs.getString(2));
		 this.setBank(rs.getString(3));
		 Customer customer = new Customer();
		 customer.setCustomerID(rs.getString(2));
		 ExternalContext context = FacesContext.getCurrentInstance().getExternalContext();
		 customer.updateCustomer();
		 context.getSessionMap().put("customer", customer);
